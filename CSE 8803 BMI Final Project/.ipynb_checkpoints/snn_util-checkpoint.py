...
class AbstractSNN_2(nn.Module):
    ...
    def __init__(self, T, embed_keys_path, null_word=None, beta=1.0):
        ...
        self.linear2 = nn.Linear(150, 50, bias=False)
        self.lif3 = snn.Leaky(beta=beta, init_hidden=True)
        self.conv3 = nn.Conv1d(16, 32, 7, bias=False)
        self.flatten2 = nn.Flatten()
        
        self.lif4 = snn.Leaky(beta=beta, init_hidden=True)
        self.linear3 = nn.Linear(1408, 91, bias=False)
...
def train_model(model, optimizer, dataset, loss_fn, T, epochs, batch_size, save_freq=None, save_path=None, scheduler=None):
    ...
    print(f'Saving model every {save_freq} epochs' if save_freq else 'WARNING: Will not save model!')
...
if __name__ == '__main__':
    ...
    # model = load_from_cnn(model, 'cnn_model-5.pth', 'cnn_model-5-max-activations.pkl')
...
#     epochs = 250
#     train_model(model, optimizer, loss_fn, T=T, epochs=epochs, batch_size=batch_size, save_freq=None, save_path=save_path,
#             scheduler=scheduler)
#     save_model(save_path, model, optimizer, epochs)
#     print(f'Saved to {save_path}')
