...
def load_from_cnn(model, cnn_model_weights, activations_path, wv=None, num_lif=4, beta=1.):
    '''
    Convert a non-spiking neural net into an SNN. Weights are loaded and normalized, such that the maximum
    activation
    '''
    ...
    conv_weights['embedding.weight'] = torch.from_numpy(wv.vectors) if wv is not None else weights['embedding.weight']
